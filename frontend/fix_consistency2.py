...
for fpath in files:
    if not (fpath.lower().endswith('workspace.jsx') or fpath.lower().endswith('index.jsx')): continue
    ...
    if 'Export CSV' not in new_content or 'relative' not in new_content:
        ...
        if header_match:
            action_block = action_block_workspace if 'workspace.jsx' in fpath.lower() else action_block_index
