...
import { MASTER_LINKS } from './masterLinks';

export function MasterHomePage() {
  const navigate = useNavigate();
  const { user } = useAuth();

  const visibleLinks = MASTER_LINKS.filter((link) => {
  ...
  const groupOrder = ['Overview', 'Operations', 'Accounting', 'Other'];
  const sectionCount = Object.keys(groupedLinks).length;
  const itemCount = visibleLinks.length;
  ...
  return (
    <div className="space-y-6">
      <Card className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div className="relative bg-gradient-to-r from-[#0f172a] via-[#1d4ed8] to-[#3b82f6] px-6 py-8 text-white md:px-8">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl space-y-4">
              ...
              <div>
                <h1 className="text-3xl font-semibold tracking-tight md:text-4xl">Master</h1>
                <p className="mt-2 max-w-2xl text-sm text-blue-50 md:text-[15px]">
                  Open staff, branch, member, accounting, and operational records from one clean hub.
                </p>
              </div>
            </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {[
          { title: 'Operations', description: 'Branches, employees, members, and committee.', count: groupedLinks.Operations?.length || 0, tone: 'emerald' },
          { title: 'Accounting', description: 'Ledgers, rates, bank accounts, demands, and exclusions.', count: groupedLinks.Accounting?.length || 0, tone: 'amber' }
        ].map((item) => (
          <Card key={item.title} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className={`inline-flex rounded-full border px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] ${toneStyles(item.tone)}`}>
                  {item.title}
                </p>
                <h2 className="mt-3 text-lg font-semibold text-slate-900">{item.count} items</h2>
