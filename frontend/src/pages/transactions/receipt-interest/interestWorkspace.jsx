    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Button type="button" variant="outline" className="gap-2" onClick={() => setFilterDropdownOpen(!filterDropdownOpen)}>
            <Filter size={14} /> Filter
          </Button>
          <div className="relative">
            <Button type="button" className="gap-2 bg-[var(--primary,#1661F6)] text-white hover:opacity-90" onClick={() => setDropdownOpen(!dropdownOpen)} onBlur={() => setTimeout(() => setDropdownOpen(false), 200)}>
            ...
            {dropdownOpen && (
              <div className="absolute right-0 top-full z-50 mt-2 w-72 origin-top-right rounded-2xl border border-slate-200 bg-white p-2 shadow-xl ring-1 ring-slate-900/5">
                ...
                >
                  ...
                  <span className="line-clamp-1">{activeItem?.label || 'Interest'}</span>
                </button>
              </div>
            )}
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {[
          ...
          { label: 'Posted', value: visibleRows.filter((row) => String(row.status || '').toLowerCase() === 'posted').length, icon: CheckCircle, color: 'text-emerald-500', bg: 'bg-emerald-50' },
      ...
      <ConfirmDialog open={Boolean(reverseTarget)} title="Reverse interest" description="This interest entry will be reversed and marked accordingly." confirmLabel="Reverse" tone="outline" onConfirm={confirmReverse} onClose={() => setReverseTarget(null)} />
    </div>
  );
}
