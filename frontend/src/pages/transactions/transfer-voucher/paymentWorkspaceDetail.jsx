...
export function TransferVoucherPaymentWorkspaceDetailPage({ sectionKey, itemKey, detailPathBase }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((error) => {
        if (!mounted) return;
        toast.error(error.message || 'Unable to load transfer voucher payment');
      })
      .finally(() => {
  ...
  async function saveVoucher(event) {
    ...
    try {
      ...
      toast.success('Transfer voucher payment updated');
      closeEditor();
    } catch (error) {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      navigate(detailPathBase || `/app/transactions/transfer-voucher/payment`);
    } catch (error) {
  ...
  const journalRows = journalLines.map((line, index) => ({
    key: line.id || index,
    cells: [line.accountCode || line.ledgerCode || line.account || '—', line.description || line.narration || '—', line.side || line.drCr || '—', toCurrency(line.amount || 0)]
  }));
  ...
  function handleDeleteAttachment(key, document) {
    ...
    api.files.remove(token, document.fileId)
      .then(() => {
        ...
        return api.banking.updateTransactionVoucher(token, record.id, { documents: nextDocuments })
          .then((response) => setRecord(response.data || { ...record, documents: nextDocuments }));
      })
      .then(() => toast.success('Attachment removed'))
      .catch((error) => toast.error(error.message || 'Unable to remove attachment'));
  }

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 text-[13px] font-medium text-slate-500 print:hidden">
        <button type="button" onClick={() => navigate(detailPathBase || `/app/transactions/transfer-voucher/payment`)} className="flex items-center gap-1.5 transition-colors hover:text-slate-900">
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex border-b border-slate-200 px-8 pt-4 overflow-x-auto hide-scrollbar">
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`relative flex items-center gap-2 whitespace-nowrap px-4 py-3 text-[14px] font-medium transition-colors ${activeTab === tab.id ? 'text-[var(--primary)]' : 'text-slate-500 hover:text-slate-700'}`}>
        ...
        <div className="space-y-6 p-8">
            ...
            <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
              <Card className="rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div className="divide-y divide-slate-100 px-6">
                  ...
                  <DetailRow label="Paid From" value={paidFromLabel} />
                </div>
              </Card>
              <Card className="rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div className="divide-y divide-slate-100 px-6">
          ...
          {activeTab === 'payment' ? (
            <Card className="rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="border-b border-slate-100 px-6 py-5">
                ...
                <p className="mt-1 text-sm text-slate-500">Ledger-wise payment posting fields from the transfer voucher workflow.</p>
              </div>
              <SimpleTable headers={['Field', 'Value']} rows={paymentRows} emptyMessage="No payment details found." />
            </Card>
          ) : null}
          ...
          {activeTab === 'journal' ? (
            <Card className="rounded-2xl border border-slate-200 bg-white shadow-sm">
              ...
              <SimpleTable headers={['Account', 'Narration', 'Side', 'Amount']} rows={journalRows} emptyMessage="No journal lines available." />
            </Card>
          ) : null}
      ...
      <Modal open={editorOpen} onClose={closeEditor} title="Edit Transfer Voucher Payment" width="min(1100px, 96vw)" footer={<div className="flex w-full justify-end gap-3">
          <Button type="button" variant="outline" onClick={closeEditor} disabled={saving}>Cancel</Button>
          <Button type="submit" form="transaction-voucher-form" className="bg-[var(--primary,#1661F6)] text-white hover:opacity-90" disabled={saving}> {saving ? 'Saving...' : 'Save'} </Button>
        </div>}>
        <div className="max-h-[80vh] overflow-y-auto pr-1">
          <TransferVoucherPaymentForm section={section} itemKey={itemKey} lookups={lookups} value={draft} setValue={setDraft} onSubmit={saveVoucher} onDocumentRemove={handleDocumentRemove} />
        </div>
...
export default TransferVoucherPaymentWorkspaceDetailPage;
